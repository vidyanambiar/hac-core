...
const VirtualizedTableBody = <D,>({
  ...
  const cellMeasurementCache = new CellMeasurerCache({
    fixedWidth: true,
    minHeight: 44,
    keyMapper: (rowIndex: number) => (data?.[rowIndex] as unknown as Record<string, Record<string, unknown>>)?.metadata?.uid ?? rowIndex,
  });

  // eslint-disable-next-line react/prop-types -- this rule has issues with React.memo
  const RowMemo: React.FC<RowMemoProps<D>> = React.memo(({ Row: RowComponent, obj }) => <RowComponent obj={obj} />);
